...
public class PackageData: IPackageData
{
    ...
    public async Task<List<Package>> GetTopPackagesAsync(int amount)
    {
        ...
        //TODO: Because Downloads are based on the collective downloads of the versions, this method might need to move to the VersionService.
        //return await _ctx.Package.Include(v => v.Versions).OrderByDescending(p => p.Downloads).ToListAsync();
    }
    ...
    public async Task<Package> CreatePackageAsync(Package newPackage)
    {
        ...
        _ctx.SaveChanges();//TODO: Change to SaveChangesAsync (not available for some reason??)
    ...
    {
        return await _ctx.Package.Include(v => v.Versions).FirstAsync(p => p.Id == packageId);
    }
